...
public class BiDirectionalJoin {
    ...
    public static class JoinReducer extends Reducer<Text, Text, Text, Text>{
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
            ...
            for (Text t : values){
                ...
                if (t_temp.startsWith("$")){
                    file1.add(new Text(t_temp.substring(1)));
                }
                else{
                    file2.add(new Text(t_temp));
                }
            }
            for(Text left_t: file1){
                for (Text right_t: file2){
                    result.set(left_t.toString()+","+right_t.toString());
